...
def plot_trajectory_analysis(waypoints, trajectory_data, trajectory_type):
    ...
    if len(waypoints) < 2:
        print("Warning: No valid waypoints found. Skipping error analysis.")
        ideal_path = np.array([])
        errors = np.array([])
    else:
        # Generate ideal path
    ...
    if len(errors) > 0 and np.all(np.isfinite(errors)):
        ...
        ax4.text(0.5, 0.5, 'No valid error data\navailable', ha='center', va='center', 
    ...
    output_file = os.path.join(script_dir, 'trajectory_analysis.png')
...
def main():
    # Log file is now in the same directory as this script
    log_file = os.path.join(os.path.dirname(__file__), "trajectory_log.txt")
