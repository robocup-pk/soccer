...
/**
 * @brief EXACT copy of Advanced's TrajPath - A path of connected trajectory segments for smooth multi-waypoint motion.
 * 
 * This implements the REAL Advanced approach for smooth trajectories:
 ...
 * 
 * Direct port of: /Advanced/modules/sumatra-pathfinder/src/main/java/edu/tigers/sumatra/pathfinder/finder/TrajPath.java
 */
class TrajPath {
private:
...
public:
    ...
    /**
     * Create a new path with the given inputs (EXACT copy of Advanced's with() method)
     */
    ...
    /**
     * Append a new segment to this path at the given time (EXACT copy of Advanced's append() method)
     */
    TrajPath append(double connectionTime, const Eigen::Vector2d& dest, double destTheta,
                   double maxVel, double maxAcc, double maxOmega, double maxOmegaAcc);
    
    /**
     * Get next destination at given time (used by Advanced's executePath)
     */
    Eigen::Vector2d getNextDestination(double t) const;
    
    // Trajectory interface (EXACT copy of Advanced's methods)
...
private:
    /**
     * Connect this path with another path at the given time (EXACT copy of Advanced's connect() method)
     */
