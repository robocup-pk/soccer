...
class OmniwheelSimulator(QMainWindow):
    def __init__(self):
        ...
        # Robot state (world coordinates) - TRUE position
        self.robot_x = 0.0
        self.robot_y = 0.0
        self.robot_theta = 0.0  # heading angle
        
        # Dead reckoning state (estimated position from wheel encoders)
        self.dr_x = 0.0
        self.dr_y = 0.0
        self.dr_theta = 0.0
        
        # Encoder simulation (ticks per revolution)
        self.ticks_per_rev = 1000  # Typical encoder resolution
        
        # Simulation parameters
        self.dt = 0.05  # 50ms update rate
        self.scale = 100  # pixels per meter for display
    ...
    def setup_ui(self):
        ...
        self.position_label = QLabel(f"True Position: ({self.robot_x:.3f}, {self.robot_y:.3f})")
        self.heading_label = QLabel(f"True Heading: {self.robot_theta:.3f} rad")
        self.dr_position_label = QLabel(f"Dead Reckoning: ({self.dr_x:.3f}, {self.dr_y:.3f})")
        self.dr_heading_label = QLabel(f"DR Heading: {self.dr_theta:.3f} rad")
        
        # Calculate position error
        pos_error = np.sqrt((self.robot_x - self.dr_x)**2 + (self.robot_y - self.dr_y)**2)
        self.error_label = QLabel(f"Position Error: {pos_error:.3f} m")
        
        status_layout.addWidget(self.position_label, 0, 0)
        status_layout.addWidget(self.heading_label, 1, 0)
        status_layout.addWidget(self.dr_position_label, 2, 0)
        status_layout.addWidget(self.dr_heading_label, 3, 0)
        status_layout.addWidget(self.error_label, 4, 0)
    ...
    def update_simulation(self):
        ...
        # TRUE ROBOT MOTION - Forward kinematics
        ...
        # DEAD RECKONING - Simulate encoder-based position estimation
        # Convert wheel velocities to "encoder ticks" and back (simulating measurement noise/quantization)
        encoder_ticks = wheel_angular_velocities * self.dt * self.ticks_per_rev / (2 * np.pi)
        # Add small amount of encoder quantization (round to nearest tick)
        encoder_ticks = np.round(encoder_ticks)
        
        # Convert back to wheel velocities (what the dead reckoning system would see)
        measured_wheel_velocities = encoder_ticks * 2 * np.pi / (self.ticks_per_rev * self.dt)
        
        # Dead reckoning inverse kinematics
        dr_body_velocities = self.wheel_radius * self.J_pinv @ measured_wheel_velocities
        
        dr_v_x_body, dr_v_y_body, dr_omega_body = dr_body_velocities
        
        # Transform to global frame using dead reckoning heading
        dr_cos_theta = np.cos(self.dr_theta)
        dr_sin_theta = np.sin(self.dr_theta)
        
        dr_v_x_global = dr_cos_theta * dr_v_x_body - dr_sin_theta * dr_v_y_body
        dr_v_y_global = dr_sin_theta * dr_v_x_body + dr_cos_theta * dr_v_y_body
        dr_omega_global = dr_omega_body
        
        # Integrate DEAD RECKONING position
        self.dr_x += dr_v_x_global * self.dt
        self.dr_y += dr_v_y_global * self.dt
        self.dr_theta += dr_omega_global * self.dt
        
        # Normalize dead reckoning angle
        self.dr_theta = np.arctan2(np.sin(self.dr_theta), np.cos(self.dr_theta))
        
        # Calculate position error
        pos_error = np.sqrt((self.robot_x - self.dr_x)**2 + (self.robot_y - self.dr_y)**2)
        
        # Update status display
        self.position_label.setText(f"True Position: ({self.robot_x:.3f}, {self.robot_y:.3f})")
        self.heading_label.setText(f"True Heading: {self.robot_theta:.3f} rad")
        self.dr_position_label.setText(f"Dead Reckoning: ({self.dr_x:.3f}, {self.dr_y:.3f})")
        self.dr_heading_label.setText(f"DR Heading: {self.dr_theta:.3f} rad")
        self.error_label.setText(f"Position Error: {pos_error:.3f} m")
...
class SimulationCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Convert robot positions to screen coordinates
        ...
        dr_screen_x = center_x + self.simulator.dr_x * self.simulator.scale
        dr_screen_y = center_y - self.simulator.dr_y * self.simulator.scale  # Flip Y
        
        # Draw TRUE robot body (blue)
        self.draw_robot(painter, robot_screen_x, robot_screen_y, self.simulator.robot_theta, 
                       QColor(0, 0, 255), "TRUE")
        
        # Draw DEAD RECKONING robot body (red)
        self.draw_robot(painter, dr_screen_x, dr_screen_y, self.simulator.dr_theta, 
                       QColor(255, 0, 0), "DR")
        
        # Draw position trail dots
        painter.setPen(QPen(QColor(0, 0, 255), 4))  # Blue for true robot
        painter.drawEllipse(int(robot_screen_x - 2), int(robot_screen_y - 2), 4, 4)
        
        painter.setPen(QPen(QColor(255, 0, 0), 4))  # Red for dead reckoning
        painter.drawEllipse(int(dr_screen_x - 2), int(dr_screen_y - 2), 4, 4)
        
    def draw_legend(self, painter, x, y):
        """Draw legend showing robot colors"""
        painter.setPen(QPen(QColor(0, 0, 0), 1))
        ...
        painter.drawText(x + 10, y + 15, "Legend:")
        ...
        painter.drawLine(x + 10, y + 30, x + 30, y + 30)
        painter.setPen(QPen(QColor(0, 0, 0), 1))
        painter.drawText(x + 35, y + 35, "True Robot")
        
        # Dead reckoning robot
        painter.setPen(QPen(QColor(255, 0, 0), 3))
        painter.drawLine(x + 10, y + 45, x + 30, y + 45)
        painter.setPen(QPen(QColor(0, 0, 0), 1))
        painter.drawText(x + 35, y + 50, "Dead Reckoning")
        
    def draw_robot(self, painter, screen_x, screen_y, theta, color, label):
        """Draw a robot at the specified position with given color"""
        ...
        painter.rotate(-theta * 180 / np.pi)  # Negative for screen coordinates
        ...
        # Draw wheels only for the true robot (to avoid clutter)
        if label == "TRUE":
            self.draw_wheels(painter, screen_x, screen_y, theta)
            
    def draw_wheels(self, painter, robot_screen_x, robot_screen_y, robot_theta):
        """Draw wheels for the robot"""
        ...
        painter.rotate(-robot_theta * 180 / np.pi)  # Negative for screen coordinates
