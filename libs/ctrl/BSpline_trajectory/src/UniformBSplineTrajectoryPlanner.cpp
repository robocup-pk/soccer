...
Eigen::Vector3d UniformBSplineTrajectoryPlanner::Update(const Eigen::Vector3d& current_pose, double current_time) {
    ...
    velocity_command = util::RotateAboutZ(velocity_command, -current_pose[2]);
...
Eigen::Vector3d UniformBSplineTrajectoryPlanner::CalculateVelocityCommand(const Eigen::Vector3d& current_pose,
    ...
    // Check if original waypoints have meaningful orientation specified
    bool has_meaningful_orientation = false;
    for (const auto& wp : waypoints_) {
        if (std::abs(wp[2]) > 1e-6) {  // If any waypoint has non-zero orientation
            has_meaningful_orientation = true;
            break;
        }
    }
    
    if (has_meaningful_orientation) {
        // Use tangent-based heading only when orientations were actually specified
        ...
        // When no meaningful orientation specified, keep current heading (no rotation)
        desired_heading = current_pose[2];
    }
...
bool UniformBSplineTrajectoryPlanner::UpdatePartialTrajectory(const Eigen::Vector3d& current_pose,
    ...
    // Calculate the active knot span
    double scaled_u = u_current * (n - p);
    int span_idx = static_cast<int>(scaled_u) + p;
    ...
    // Store original control points for smoothing
    std::vector<Eigen::Vector3d> original_cps(control_points_.begin() + start_idx, 
                                              control_points_.begin() + end_idx);
    
    // Apply soft correction with exponential decay
    for (int i = start_idx; i < end_idx; ++i) {
        // Distance from the start of update window
        double window_pos = static_cast<double>(i - start_idx) / num_control_points_to_update;
    ...
    // Smooth the transition at boundaries to avoid discontinuities
    if (start_idx > p && start_idx < n - 1) {
        control_points_[start_idx] = 0.7 * control_points_[start_idx] + 
                                     0.3 * original_cps[0];
    }
    if (end_idx > 0 && end_idx < n - p) {
        control_points_[end_idx - 1] = 0.7 * control_points_[end_idx - 1] + 
                                       0.3 * original_cps[num_control_points_to_update - 1];
    }
    
    // Apply boundary constraints
    ApplyBoundaryConstraints();
    
    // Clear derivative cache but don't recalculate arc length
    derivative_control_points_.clear();
