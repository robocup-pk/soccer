...
void print_detection(const SSL_DetectionFrame& frame) {
  ...
  for (int i = 0; i < frame.balls_size(); ++i) {
    ...
    std::cout << " confidence=" << ball.confidence();
    std::cout << std::endl;
  }
  ...
  for (int i = 0; i < frame.robots_yellow_size(); ++i) {
    ...
    std::cout << "  Yellow " << i << ": id=" << robot.robot_id() << " x=" << robot.x()
              << " y=" << robot.y();
  ...
  for (int i = 0; i < frame.robots_blue_size(); ++i) {
    ...
    std::cout << "  Blue " << i << ": id=" << robot.robot_id() << " x=" << robot.x()
              << " y=" << robot.y();
    // If you have velocity fields, print them here
    std::cout << std::endl;
  }
...
int main() {
  ...
  receiver.setDetectionCallback([](const SSL_DetectionFrame& frame) { print_detection(frame); });
