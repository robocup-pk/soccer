  ...
  for (auto& obj : soccer_objects) {
    if (obj.name == "ball") {
      ...
      if ((left >= left_goal_x1 && left <= left_goal_x2 && top <= left_goal_y1 &&
        ...
        goal_scored = true;
        team_with_ball = 1;
        std::cout << "RED TEAM SCORED! " << "red team score: " << team_two_score << std::endl;
      } else if (right >= right_goal_x1 && right <= right_goal_x2 && top <= right_goal_y1 &&
                 bottom >= right_goal_y2) {
        team_one_score++;
        goal_scored = true;
        team_with_ball = 2;
        std::cout << "YELLOW TEAM SCORED! " << "yellow team score: " << team_one_score
                  << std::endl;
      }
      ...
      if (goal_scored) {
        ...
        state = Game::Kickoff;
        std::cout << "GAME STATE IS KICKOFF" << std::endl;
...
void ref::Game::UpdateGameState(std::vector<state::SoccerObject>& soccer_objects) {
  if (state == Kickoff) {
    ...
    if (ball_displacement.norm() > 0.05) {
      state = Run;
      std::cout << "GAME STATE IS RUN" << std::endl;
    }
  }
}
